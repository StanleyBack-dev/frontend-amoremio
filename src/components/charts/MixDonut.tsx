...
// clear every CVD/contrast gate together; a 5th+ entry never gets a new
...
// Shared donut-with-legend used by every "mix" chart (canais, produtos,
// clientes) — each caller just reshapes its rows into {key, label, value}.
export default function MixDonut({
  ...
  const otherTotal = tail.reduce((sum, row) => sum + row.value, 0);
                  ...
                  rows={[
                    {
                      key: slice.key,
                      label: slice.label,
                      value: `${valueFormat(slice.value)} (${((slice.value / total) * 100).toFixed(0)}%)`,
                      color: slice.color,
                    },
                  ]}
                />
